...
class PlotHeatmap:
    ...
    def plot(self):
        ...
        plt.xlabel("Métricas de evaluación", fontsize=14)
        plt.ylabel("Modelos entrenados", fontsize=14)
        plt.yticks(rotation=0)
        cbar = ax.collections[0].colorbar
        cbar.ax.tick_params(labelsize=13)
        plt.tight_layout(h_pad=0.9)
        output_root = f"{self.local_root}/results/metrics_results/metrics_plots"
        # output_figure = f"[es]_heatmap_{self.algorithm}_{self.quartile}.tiff"
        output_figure = f"[es]_heatmap_{self.algorithm}_{self.quartile}.png"
        plt.savefig(
